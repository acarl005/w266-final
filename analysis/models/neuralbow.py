    ...
    for i, hdim in enumerate(hidden_dims):
        ...
        if dropout_rate > 0:
            h_ = tf.layers.dropout(h_,
                                   ...
                                   name=("Hidden_dropout_%d"%i))  # replace with dropout applied to h_
...
def softmax_output_layer(h_, labels_, num_classes):
    ...
    with tf.name_scope("Softmax"):
        #### YOUR CODE HERE ####
        loss_ = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=labels_,logits=logits_))  # replace with mean cross-entropy loss over batch
